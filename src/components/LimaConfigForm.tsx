    ...
    <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-zinc-800 h-full overflow-hidden">
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1 min-h-0 pb-8 [scrollbar-gutter:stable]">
          <div>
            <label className="text-[10px] uppercase text-zinc-600 font-bold block mb-1">CPUs</label>
            <input
              type="number"
              min="1"
              max="16"
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 space-y-6 overflow-y-auto flex-1 min-h-0 pb-8 [scrollbar-gutter:stable]">
            ...
            {showImages && (
              <div className="space-y-3 animate-in slide-in-from-top-2 fade-in duration-200">
                {(parsedConfig.images as Image[] | undefined)?.map((img: Image, i: number) => (
                  <div
                    key={i}
                    className="bg-zinc-950/50 border border-zinc-800 rounded p-2 space-y-2 relative group"
                  >
                    <button
            ...
            {showMounts && (
              <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-200">
                {(parsedConfig.mounts as Mount[] | undefined)?.map((mnt: Mount, i: number) => (
                  <div
                    key={i}
                    className="bg-zinc-950/50 border border-zinc-800 rounded p-2 flex items-center gap-2 group"
                  >
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            updateArrayItem("mounts", i, "writable", e.target.checked)
                          }
                        ...
                        <label
                          htmlFor={`mnt-write-${i}`}
                          className="text-[10px] text-zinc-500 uppercase select-none cursor-pointer"
                        >
                          Writable
                        </label>
                      </div>
          ...
          <div className="space-y-2">
            ...
            {showCopyToHost && (
              <div className="space-y-3 animate-in slide-in-from-top-2 fade-in duration-200">
                {(parsedConfig.copyToHost as CopyToHost[] | undefined)?.map(
                  (rule: CopyToHost, i: number) => (
                    <div
                      key={i}
                      className="bg-zinc-950/50 border border-zinc-800 rounded p-2 space-y-2 relative group"
                    >
                      ...
                      <div className="grid grid-cols-[30px_1fr] items-center gap-2">
                        ...
                        <input
                          ...
                          value={rule.guest || ""}
                          onChange={(e) =>
                            updateArrayItem("copyToHost", i, "guest", e.target.value)
                          }
                        />
                      </div>
                      <div className="grid grid-cols-[30px_1fr] items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 pl-9.5">
                        <input
                          ...
                          onChange={(e) =>
                            updateArrayItem("copyToHost", i, "deleteOnStop", e.target.checked)
                          }
                          className="accent-zinc-500 w-3 h-3"
                          id={`cp-del-${i}`}
                        />
                        <label
                          htmlFor={`cp-del-${i}`}
                          className="text-[9px] text-zinc-500 uppercase select-none cursor-pointer"
                        >
                          Delete on Stop
                        </label>
                      </div>
            ...
            {effectiveShowScripts && (
              <div className="space-y-4 animate-in slide-in-from-top-2 fade-in duration-200">
                {parsedConfig.provision &&
                  (parsedConfig.provision as Provision[]).map(
                    (script: Provision, index: number) => (
                      <div
                        key={index}
